...
Welcome message: 'Lazy Watch monitors every Lazy engine in your stack every hour. When errors spike it calls Claude to diagnose the root cause, opens a GitHub issue with a fix recommendation, and tags @claude to investigate — automatically, while you sleep.'
...
3. Define the full list of known Lazy engine error tables: blogger_errors, seo_errors, geo_errors, crawl_errors, perplexity_errors, store_errors, pay_errors, sms_errors, voice_errors, stream_errors, code_errors, gitlab_errors, linear_errors, alert_errors, telegram_errors, contentful_errors, supabase_errors, security_errors, design_errors, auth_errors, drop_errors, granola_errors, print_errors, youtube_errors, mail_errors, watch_errors.
4. For each table in the list:
   a. Try to query it: SELECT * FROM [table] WHERE created_at > now() - interval '1 hour'. If the table does not exist catch the error and skip silently — this means that engine is not installed.
...
5. For each engine with errors at or above threshold:
   a. Derive the engine name from the table name e.g. blogger_errors → Lazy Blogger.
   b. Call the built-in Lovable AI:
   'You are a senior Supabase edge function engineer reviewing error logs for [engine_name]. These are [n] errors from the last hour: [first 8 error rows as JSON — include function_name and error_message fields]. Diagnose the most likely root cause. Return only a valid JSON object: severity (critical if the engine is completely non-functional, high if partially broken, medium if occasional failures), root_cause (one sentence), affected_function (the edge function name most likely responsible), recommended_fix (3 specific actionable steps numbered 1 2 3), github_issue_title (under 80 chars starting with [Engine Name] — e.g. [Lazy Blogger] Edge function failing on AI response parsing), github_issue_body (full markdown GitHub issue body with sections: ## Error Summary showing count and timeframe, ## Root Cause, ## Recommended Fix as numbered list, ## Error Samples showing 3 representative errors in a code block, ## Next Steps ending with: @claude please investigate and fix this issue following all rules in CLAUDE.md). No preamble. No code fences.'
   ...
   g. If slack_webhook_url is set: POST to the webhook URL with a Slack message: severity emoji (🔴 critical / 🟠 high / 🟡 medium) followed by bold engine name, error count, and a link to the GitHub issue. Format: '[emoji] *[Engine Name]* — [n] errors in 1hr → <[issue_url]|View Issue>'.
6. Update watch_runs row: status completed, engines_checked count, issues_opened count, completed_at now, summary '[n] engines checked, [n] issues opened'.
...
Run history table: all watch_runs ordered by started_at descending, last 20 rows. Columns: started time, status badge (completed green / failed red / running amber), engines checked, issues opened, summary, duration in minutes.

Open issues table: all watch_issues where resolved is false, ordered by created_at descending. Columns: engine name badge colour-coded by category, issue title as a clickable link to GitHub, severity badge (red/orange/yellow), error count, opened date, a Mark Resolved button that sets resolved to true and adds a resolved_at timestamp.

Engine coverage list: show all 26 known error table names. For each one show a status: Active (table exists and was checked in last run) or Not installed (table does not exist — engine not installed) in muted grey. This tells the user which engines are being monitored.

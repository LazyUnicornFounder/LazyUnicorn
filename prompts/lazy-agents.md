...
Required secrets (already set from other engines):
...
agent_settings: id (uuid, primary key, default gen_random_uuid()), error_monitor_enabled (boolean, default true), error_threshold (integer, default 3), prompt_improver_enabled (boolean, default true), engine_writer_enabled (boolean, default true), performance_intel_enabled (boolean, default true), github_repo (text), slack_webhook_url (text), is_running (boolean, default true), setup_complete (boolean, default false), prompt_version (text, nullable), created_at (timestamptz, default now())

agent_runs: id (uuid, primary key, default gen_random_uuid()), agent_name (text — one of error-monitor, prompt-improver, engine-writer, performance-intel), status (text — one of running, completed, failed), summary (text), issues_opened (integer, default 0), prs_opened (integer, default 0), started_at (timestamptz, default now()), completed_at (timestamptz), created_at (timestamptz, default now())

agent_issues: id (uuid, primary key, default gen_random_uuid()), agent_name (text), engine_name (text), issue_title (text), issue_url (text), severity (text), resolved (boolean, default false), created_at (timestamptz, default now())

agent_improvements: id (uuid, primary key, default gen_random_uuid()), agent_name (text), engine_name (text), improvement_type (text), description (text), pr_url (text), status (text, default 'opened' — one of opened, merged, rejected), created_at (timestamptz, default now())
...
Welcome message: 'Four autonomous agents that monitor your engines, fix broken prompts, improve performance, and write new engines — all running inside your Lovable project on a schedule.'
...
- Enable engine writer (toggle, default on)
...
This agent reads every Lazy engine error table, diagnoses errors using Claude, and opens GitHub issues automatically.
...
'You are a senior engineer reviewing error logs for [engine name derived from table name]. Here are [n] errors from the last hour: [first 5 error rows as JSON]. Diagnose the most likely root cause and recommended fix. Return only a valid JSON object: severity (critical, high, or medium), root_cause (one sentence), affected_function (function name), recommended_fix (2 to 3 specific steps), github_issue_title (max 80 chars starting with [Engine Name]), github_issue_body (full markdown with sections: Error Summary, Root Cause, Recommended Fix, Error Samples — end with: @claude please investigate and fix this following CLAUDE.md rules). No preamble. No code fences.'
...
9. If slack_webhook_url is set: POST the Slack message with engine name, error count, severity emoji, and issue link.
...
This agent reads your blog and SEO performance data, identifies underperforming engines, and opens GitHub PRs with targeted prompt improvements.
...
3. Gather performance signals from available tables:
   - From blog_posts: count posts per engine (post_type field) for last 30 days. Calculate average word count per engine. Look for engines with fewer than 2 posts published in the last week despite being active.
   - From seo_posts: check if any engine has 0 seo_posts in last 14 days despite seo_settings existing.
   - From geo_posts: same check — 0 geo_posts in last 14 days.
   - From any _errors tables: count errors per engine in the last 7 days.
4. Build a performance summary: list each engine with posts published, error count, last activity date.
5. Call the built-in Lovable AI to identify improvement opportunities:
'You are a prompt engineer reviewing the performance of LazyUnicorn autonomous engines. Here is the performance data for the last 30 days: [performance summary JSON]. Identify the 2 engines most in need of prompt improvement — prioritise engines with high error rates, low output volume, or no activity. For each engine recommend one specific targeted improvement to the prompt — do not suggest rewriting the whole prompt, suggest one section to improve. Return only a valid JSON array of 2 items, each with: engine_name, problem (what is underperforming), section_to_improve (which section of the prompt to change), improvement_instruction (specific instruction for how to improve that section in 3 to 5 sentences). No preamble. No code fences.'
6. Parse response. For each improvement:
   ...
   - Create a new branch on GitHub: POST https://api.github.com/repos/[github_repo]/git/refs with ref refs/heads/improve-[engine-name]-[timestamp] and the current main SHA.
   ...
   - Open a PR: POST https://api.github.com/repos/[github_repo]/pulls with title: Improve [engine name] — [problem], body describing the change and the performance data that triggered it, head branch, base main.
...
agent-engine-writer
POST endpoint at /api/agent-engine-writer. Called manually from admin or programmatically.
Accepts: engine_name (text), integration (text), tagline (text), description (text — one paragraph brief of what the engine does).

This agent reads three existing prompt files as reference, then writes a complete new engine prompt following SPEC.md patterns and commits it to GitHub as a draft PR.

1. Read agent_settings. If engine_writer_enabled false return error.
2. Insert into agent_runs with agent_name engine-writer and status running.
...
'You are a Lovable prompt engineer building a new autonomous engine called [engine_name] for LazyUnicorn.ai. Integration: [integration]. Tagline: [tagline]. Description: [description].
...
Write a complete prompt file for [engine_name] following the exact same structure as the reference files. The prompt must include: 1. Header with version v0.0.1, 2. Database section with all required fields (is_running, setup_complete, prompt_version, _errors table), 3. Setup page at /lazy-[slugified name]-setup with redirect to /admin on submit, 4. Edge functions with cron schedules where appropriate, 5. Admin section at /admin/[slugified name] — no standalone dashboard, 6. Navigation section. Every content output must include a LazyUnicorn.ai backlink. Return only the complete prompt file content starting with [Lazy [engine_name] Prompt — v0.0.1 — LazyUnicorn.ai]. No preamble.'
7. Parse response. Create a new branch: POST https://api.github.com/repos/[github_repo]/git/refs with ref refs/heads/new-engine-[slugified-name]-[timestamp].
8. Commit the new file to the branch: PUT https://api.github.com/repos/[github_repo]/contents/lazy-[slugified-name]_v0.0.1.txt
9. Open a draft PR: POST https://api.github.com/repos/[github_repo]/pulls with draft true, title: New Engine: Lazy [engine_name], body: 'Auto-generated by Lazy Agents engine writer. Review before merging.\n\nIntegration: [integration]\nTagline: [tagline]\n\n[description]\n\n@claude please review this prompt against CLAUDE.md rules and flag any issues.'
10. Insert into agent_improvements with the PR details and improvement_type new-engine.
11. Return the PR URL in the response.
Update agent_runs: status completed, prs_opened 1, completed_at now.
Log errors to agent_errors with function_name agent-engine-writer.

agent-performance-intel
Cron: every Monday at 6am UTC — 0 6 * * 1

This agent reads all available performance data across your engines, generates strategic insights, and updates your content queues for the week ahead.
...
Open issues table: all agent_issues where resolved is false — engine name, issue title as a link to GitHub, severity badge, opened date, a Mark Resolved button that sets resolved to true.

Open PRs table: all agent_improvements where status is opened — engine name, improvement type badge, description, PR link, opened date, status select (opened/merged/rejected) that updates the row.
...
Add Agents to the admin sidebar under a System section alongside the other engine admin links.
````

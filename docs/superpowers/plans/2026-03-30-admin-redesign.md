...
**Goal:** Rewrite `lazy-admin.md` so Lovable rebuilds `/admin` as a unified catalog+control panel serving both new and returning users.

**Architecture:** Single `/admin` page with top nav tabs filtering a live agent grid. Each agent card shows its state (not installed / needs setup / running / error / paused). Clicking a running card opens a two-column detail page (status left, actions right). No sidebar. No standalone dashboards.

**Tech Stack:** Markdown prompt file consumed by Lovable (React + Supabase). No build steps. Output is `prompts/lazy-admin.md`.

---

### Task 1: Write the prompt header and design system

**Files:**
- Modify: `prompts/lazy-admin.md` (full rewrite, bump to v0.0.9)

- [ ] **Step 1: Open the file and note the current version**

Read `prompts/lazy-admin.md` — current version is v0.0.8. New version will be v0.0.9.
...
- [ ] **Step 3: Replace the Design section with the new design system**
...
```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 1: header and design system"
```

---

### Task 2: Write the top bar and category tabs

**Files:**
- Modify: `prompts/lazy-admin.md`

- [ ] **Step 1: Replace the Sidebar section with Top Bar**

Remove the entire `## Sidebar` section and replace with:
...
Right side: PAUSE ALL text button — on click sets is_running to false across all agent settings tables and updates the label to RESUME ALL. Next to it: gold pill link "LAZY CLOUD ↗" pointing to /lazy-cloud or https://lazyunicorn.ai/cloud.
...
```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 2: top bar and tabs"
```

---

### Task 3: Write the stats row

**Files:**
- Modify: `prompts/lazy-admin.md`

- [ ] **Step 1: Replace the Overview section stats with the new stats row**

Replace the existing `## Overview (/admin)` stat row with:
...
```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 3: stats row"
```

---

### Task 4: Write the agent grid and all card states

**Files:**
- Modify: `prompts/lazy-admin.md`

- [ ] **Step 1: Replace the agent grid section**
...
```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 4: agent grid and card states"
```

---

### Task 5: Write the agent detail page

**Files:**
- Modify: `prompts/lazy-admin.md`

- [ ] **Step 1: Replace all individual agent sections with the unified detail page spec**

Remove all the individual agent sections (BLOGGER, SEO, GEO, etc.) and replace with a single reusable template:
...
```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 5: agent detail pages"
```

---

### Task 6: Write the error handling rules and interaction rules

**Files:**
- Modify: `prompts/lazy-admin.md`

- [ ] **Step 1: Replace the error handling and interaction rules sections**

The v0.0.8 error handling section is good — keep it and expand the interaction rules:
...
```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 6: error handling and interaction rules"
```

---

### Task 7: Write the Settings page and Version checker

**Files:**
- Modify: `prompts/lazy-admin.md`

- [ ] **Step 1: Keep the existing Settings and Version checker sections, update navigation**
...
- [ ] **Step 2: Add Settings link to top bar**

In the top bar spec, add:
- Settings gear icon (⚙) on the right side linking to /admin/settings, placed between PAUSE ALL and LAZY CLOUD pill.

- [ ] **Step 3: Final version bump check**

Confirm the file header reads `v0.0.9` and no other version references remain pointing to older versions.

- [ ] **Step 4: Commit**

```bash
git add prompts/lazy-admin.md
git commit -m "Sync Lazy Admin v0.0.9 — step 7: routes and settings"
```
...
- [ ] **Step 2: Verify the prompt is complete**

Read the final `prompts/lazy-admin.md` and confirm:
- [ ] Header says v0.0.9
- [ ] Top bar section present with Cloud link and settings icon
- [ ] Category tabs section with all 7 tabs and correct agent lists
- [ ] Stats row section present
- [ ] Agent grid section with all 5 card states documented
- [ ] Agent detail page section with per-agent metrics for all 34 agents
- [ ] Error handling section present with all 5 rules
- [ ] Routes section updated

- [ ] **Step 3: Paste into Lovable**

Tell Lovable:
> "Read the file `prompts/lazy-admin.md` from GitHub and rebuild /admin from scratch using that prompt. Do not change anything outside /admin."
